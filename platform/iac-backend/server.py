...
# ====================================================================
def generate_tfvars(layer, vault_data):
    lines = []
    common_vars = {
        "aws_region": vault_data.get("aws_region", AWS_REGION),
        "aws_profile": vault_data.get("aws_profile", "root-lab-2"),
        "environment": vault_data.get("environment", "dev"),
    }
    layer_vars = {
        "network": {
            "vpc_name": vault_data.get("vpc_name", f"eks-lab-vpc-{vault_data.get('environment','dev')}"),
            "vpc_cidr": vault_data.get("vpc_cidr", "10.0.0.0/16"),
            "private_subnets": vault_data.get("private_subnets", ["10.0.1.0/24", "10.0.2.0/24"]),
            "public_subnets": vault_data.get("public_subnets", ["10.0.101.0/24", "10.0.102.0/24"]),
        },
        "compute": {
            "cluster_name": vault_data.get("cluster_name", "eks-devops-lab"),
            "node_instance_types": vault_data.get("node_instance_types", ["c7i-flex.large"]),
            "node_desired_size": vault_data.get("node_desired_size", 2),
            "node_min_size": vault_data.get("node_min_size", 1),
            "node_max_size": vault_data.get("node_max_size", 3),
        },
        "jenkins": {
            "jenkins_instance_type": vault_data.get("jenkins_instance_type", "m7i-flex.large"),
            "jenkins_root_volume_size": vault_data.get("jenkins_root_volume_size", 30),
            "key_name": vault_data.get("key_name", "KeyPair-2"),
        },
        "rancher": {
            "rancher_instance_type": vault_data.get("rancher_instance_type", "c7i-flex.large"),
            "rancher_root_volume_size": vault_data.get("rancher_root_volume_size", 50),
            "key_name": vault_data.get("key_name", "KeyPair-2"),
        },
    }
    lines.append(f"# Auto-generated by IaC AIO from Vault")
    lines.append(f"# Env: {vault_data.get('environment')}, Layer: {layer}")
    lines.append("")
    for k, v in common_vars.items():
        lines.append(f'{k} = {json.dumps(v)}')
    if layer in layer_vars:
        for k, v in layer_vars[layer].items():
            lines.append(f'{k} = {json.dumps(v)}')
    return "\n".join(lines) + "\n"
...
def stream_terraform(wfile, env, layer, action):
    """Chạy terraform và stream từng dòng output qua SSE."""
    ...
    try:
        ...
        # ─── Cleanup: xóa terraform.tfvars sau khi chạy xong ───
        tfvars_file = os.path.join(layer_dir, "terraform.tfvars")
        if os.path.isfile(tfvars_file):
            os.remove(tfvars_file)
            sse_send(wfile, "🧹 Auto-cleaned terraform.tfvars")
...
class IacHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/iac-terraform-stream":
            ...
            for layer in layers:
                # Ghi tfvars
                tfvars_content = generate_tfvars(layer, vault_data)
                layer_dir = os.path.join(INFRA_ROOT, f"envs/{env}/{layer}")
                if os.path.isdir(layer_dir):
                    with open(os.path.join(layer_dir, "terraform.tfvars"), "w") as f:
                        f.write(tfvars_content)
                    sse_send(wfile, f"📝 Generated terraform.tfvars for {layer}")
                
                # Chạy terraform streaming
                stream_terraform(wfile, env, layer, action)
                sse_send(wfile, "")
